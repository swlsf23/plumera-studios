---
title: "About Plumera Studios | Plumera"
description: Learn about Plumera Studios, an independent studio creating language-learning resources, interactive tools, and word games.
eyebrow: Studio
draft: false
related:
  - title: What's new
    href: /en/learn-french/whats-new/
  - title: CEFR levels
    href: /en/cefr/
  - title: Privacy
    href: /en/privacy/
---

# About Plumera Studios

Plumera Studios is a small, independent studio focused on language learning and word games.

## Who we are

What we make should be clear, useful, and enjoyable to use: lessons, reference material, interactive content, and games. That means cutting unnecessary complexity, making information easy to navigate, and keeping the interface quiet so the language stays front and center.

We’re small by design, which gives us the freedom to choose the right approach, experiment with ideas, and pay attention to the details.

## What we do

We create learning resources and interactive tools for people who are curious about language.

Right now, our focus is French. We’re developing lessons, reference material, and interactive resources designed to help learners understand how the language works and build confidence using it. That includes [Verb of the Week](/en/learn-french/votw/), articles on grammar and idioms, and guides such as [CEFR levels](/en/cefr/) and [language certification exams](/en/language-certification-exams/).

Spanish is next, with more languages and language-focused projects to follow as Plumera grows.

Alongside language learning, we’re also interested in words simply because they’re fun. Word games and other playful experiments with language are part of what we want Plumera Studios to become. Our long-term goal is to provide fully interactive and adaptive language learning apps.

## Learning with Plumera

We organize content by [CEFR level](/en/cefr/) so you can work at a pace that matches where you are. Each page targets a level. We do not try to cover every meaning, construction, and expression in one place.

Start with material for your current level, then come back to related topics as you progress. Higher-level pages build on what you already know and bring in more advanced uses when you are ready for them.

## What to expect

Plumera is still growing, and you’ll see new things appear regularly.

Expect new lessons and learning resources, improvements to the tools already here, and experiments with new ways to learn and interact with language. Some will be practical, some more playful, and some may simply begin with an idea we think is worth trying.

We’re not trying to build everything at once. New projects will arrive gradually, alongside continued improvements to the things already here.

## Stay in touch

Questions or feedback: [hello@plumerastudios.com](mailto:hello@plumerastudios.com).
